...
Vue.component('dashboard', {
    ...
    methods: {
        loadStatistics: function() {
            ...
            self.loading = true;
            self.loadError = '';
            fetch('/v1/dashboard', { credentials: 'same-origin' })
                .then(function(r) {
                    if (!r.ok) return r.json().then(function(d) { throw new Error(d.detail || self.$t('node.dashboard.load_error')); });
                    return r.json();
                })
                .then(function(data) {
                    self.stats = {
                        users: data.users_count != null ? data.users_count : '—',
                        managers: data.managers_count != null ? data.managers_count : '—',
                        wallets: data.wallets_count != null ? data.wallets_count : '—',
                        arbiterWallets: data.arbiter_wallets_count != null ? data.arbiter_wallets_count : '—'
                    };
                    self.loading = false;
                })
                .catch(function(err) {
                    self.loadError = err.message || self.$t('node.dashboard.load_error');
                    self.loading = false;
                });
        }
    },
    ...
    <div class="p-8 bg-zinc-50/50 flex-1 overflow-y-auto">
      <div class="max-w-7xl mx-auto">
        <nav class="text-[13px] text-blue-600 font-semibold mb-6">[[ $t('node.sidebar.dashboard') ]]</nav>
        <div class="bg-white rounded-2xl shadow-sm border border-zinc-200/60 overflow-hidden">
          <div class="p-5 border-b border-zinc-100 bg-zinc-50/30 flex items-center gap-2">
            ...
            <span class="text-[13px] font-bold text-zinc-800 uppercase tracking-tight">[[ $t('node.page.dashboard') ]]</span>
          </div>
          <div class="p-10">
            <h2 class="text-2xl font-bold text-zinc-900 mb-8 tracking-tight">[[ $t('node.dashboard.welcome') ]]</h2>
            <div v-if="isNodeInitialized" class="mb-6 rounded-xl bg-emerald-50 border border-emerald-100 p-4 text-[13px] font-medium text-emerald-900">
              [[ $t('node.dashboard.node_initialized') ]]
            </div>
            <div v-else class="mb-6 rounded-xl bg-amber-50 border border-amber-100 p-4 text-[13px] font-medium text-amber-900">
              [[ $t('node.dashboard.node_not_initialized') ]]
            </div>
            <div class="mb-6">
              <h3 class="text-[13px] font-bold text-zinc-800 uppercase tracking-[0.1em] mb-6">[[ $t('node.dashboard.statistics') ]]</h3>
              <p v-if="loadError" class="text-red-600 text-[13px] mb-3">[[ loadError ]]</p>
              <p v-if="loading" class="text-zinc-500 text-[13px] mb-4">[[ $t('node.loading') ]]</p>
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div class="flex-1 bg-white border border-blue-100 rounded-xl p-8 flex flex-col items-center justify-center text-center transition-all duration-200 hover:-translate-y-1 hover:shadow-lg">
                  ...
                  <div class="text-[13px] font-medium text-zinc-500 uppercase tracking-wide">[[ $t('node.dashboard.users') ]]</div>
                </div>
                <div class="flex-1 bg-white border border-sky-100 rounded-xl p-8 flex flex-col items-center justify-center text-center transition-all duration-200 hover:-translate-y-1 hover:shadow-lg">
                  ...
                  <div class="text-[13px] font-medium text-zinc-500 uppercase tracking-wide">[[ $t('node.dashboard.managers') ]]</div>
                </div>
                <div class="flex-1 bg-white border border-emerald-100 rounded-xl p-8 flex flex-col items-center justify-center text-center transition-all duration-200 hover:-translate-y-1 hover:shadow-lg">
                  ...
                  <div class="text-[13px] font-medium text-zinc-500 uppercase tracking-wide">[[ $t('node.dashboard.wallets') ]]</div>
                </div>
                <div class="flex-1 bg-white border border-amber-100 rounded-xl p-8 flex flex-col items-center justify-center text-center transition-all duration-200 hover:-translate-y-1 hover:shadow-lg">
                  ...
                  <div class="text-[13px] font-medium text-zinc-500 uppercase tracking-wide">[[ $t('node.dashboard.arbiter_wallets') ]]</div>
                </div>
              </div>
            </div>
          </div>
